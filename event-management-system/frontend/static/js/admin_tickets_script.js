    ...
    tbody.innerHTML = paginatedBookings.map(b => `
        <tr class="hover:bg-white/5 transition">
            ...
            <td class="px-4 py-4 text-white text-sm flex items-center gap-2">
                <img src="${b.user?.profile_image || 'https://i.pravatar.cc/32?img=1'}" class="w-6 h-6 rounded-full">
            ...
            <td class="px-4 py-4 text-white/60 text-sm">${formatDate(b.booked_at)}</td>
        </tr>
    `).join('');
